...
# Project harness rules — ryankrol.co.uk

> This project's own accumulated harness conventions and **institutional incident log**. Some
> sections intentionally elaborate, with project-specific detail and dated history, on the generic
> rules in the pristine `.harness/CLAUDE.md` above. Paths updated for the canonical layout
> (`scripts/` `config/` `tracking/` `ledgers/` `docs/`); deploy now runs via
> `custom/hooks/on-drained.sh` (was inlined in `loop.sh` pre-canonical).
...
`.harness/scripts/loop.sh` deploys unconditionally, on its own, whenever `select_task()` finds **nothing
...
**Mechanism:** when `select_task()` finds nothing eligible, `loop.sh` fires the `on-drained` lifecycle
hook (`.harness/custom/hooks/on-drained.sh`), which calls
`site_touched_since_last_deploy()`, which reads `.harness/custom/last-deploy.json` (a small, git-tracked
...
originally established by `T193`'s spec, run by the `on-drained` hook rather than via a task. On a
confirmed-200 success, it writes/commits/pushes the new `sha` into `.harness/custom/last-deploy.json` itself
...
`/mark-task-failed` command or `.harness/scripts/mark-failed.sh <TNNN> "<reason>"` (the dashboard's "Mark
...
(not via `.harness/scripts/loop.sh`), **that interactive work IS the completion mechanism.** The whole point
...
`.harness/scripts/loop.sh <TNNN>` to "make it official" or get it through CI-gating. The loop exists to
...
**Prevention — run `.harness/scripts/check-task-scope.sh` before finalizing any new task(s):**
```sh
.harness/scripts/check-task-scope.sh          # scan every pending, non-needs-human task
.harness/scripts/check-task-scope.sh T171     # scan one task
```
...
- **2026-06-26 — ROOT CAUSE FOUND + FIXED: `mark_done`/`block_task` silently failed to commit
  ...
  `mark_done` did `git add "$BACKLOG" "$WORKLOG" "$OUTCOMES" "$FAILURES"`, but `.harness/ledgers/failures.jsonl`
...
- **2026-07-02 — Force-running `.harness/scripts/loop.sh TNNN` to "close out" a task already built
  ...
  the loop was force-run against it (`.harness/scripts/loop.sh T106`). Two things went wrong at once: (1)
