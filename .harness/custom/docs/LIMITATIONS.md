...
# Project trade-offs — ryankrol.co.uk

> This project's own limitations log (the generic template's rows are replaced by these). Paths
> are canonical-layout; the pre-push guard's project patterns live in `custom/sensitive-paths.txt`.
