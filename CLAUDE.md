...
| `src/pages/index.js` | Home: tagline, `NowPlaying` widget, the gold **Top of mind** panel (`TopOfMind` component — the `/tom` blurb, collapsed to a fixed 3-line clamp with a 'See more' toggle; while loading it shows `TopOfMindSkeleton` at the exact same clamp height so there's no layout shift, and renders nothing if the note is absent/expired), 'the collection wall' (a random 18-item mosaic sampled fresh from movies/tv/books/albums/vinyl on each page load, each tile linking to its content type's list page), a 'Hot takes' preview panel in the rail column showing the 3 most recent hot takes with a link to the full `/hot-takes` list, link grid to all sections |
...
| `BookRatingsV4` | `id` (random UUID, assigned once, never regenerated) | `{ id, title, author, rating(0–5), review_text, date 'DD-MM-YYYY' }` plus optional search fields: `source` (`'googlebooks'` or `'hardcover'`), `coverUrl` (provider cover URL), `volumeId` (provider ID), `bookAuthors`, `firstPublishedYear`, `isbn`, `subjects`, `pageCount`, `publisher`, `editedDate` (`'DD-MM-YYYY'`, set on every edit). When `source='hardcover'`, also: `hardcoverSynopsis`, `hardcoverSlug`, `hardcoverRating`, `seriesName`, `seriesPosition` — fetched from Hardcover GraphQL API at confirmation time, each omitted if absent upstream. Replaces the old title+author-keyed `BookRatingsV3` (left at rest, unused, for rollback). Historical rows may still carry a legacy `olid`/`coverId`/`source:'openlibrary'` from before the Open Library search path was removed (T077) and migrated (T079/T080) — new reviews never write them |
...
- **Visual changes are confirmed with real screenshots, not just a passing build.** `node
  scripts/visual-check.mjs` renders every page AND every interaction flow hermetically (synthetic
  `/api/*` fixtures, no network) and screenshots them to `scripts/visual-out/` (+ `manifest.json`) —
  because lint/test/build can all pass while an element sits in the DOM unpainted (this shipped once:
  T273, Markdown that never rendered). When you change a page/component/style, **run it and LOOK** at
  the shots relevant to your change (use `manifest.json`'s `covers`). Keep `scripts/_visual-harness.mjs`
  current (it's a living artifact): a new page/API-shape → update `PAGES`/fixtures; **a new interactive
  state (sort/filter/toggle) → add a `FLOWS` entry** in the same change. The harness auto-runs this for
  `page`/`style`/`ui` tasks via `VISUAL_VERIFY_HOOK`. Interactive behaviour is *also* unit-tested with
  React Testing Library (`*.test.js`) — that layer runs in CI, the visual check does not.
