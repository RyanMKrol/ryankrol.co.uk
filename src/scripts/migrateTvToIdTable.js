/**
 * Migration script to copy TV review data from TelevisionRatingsV3 (keyed on title)
 * into a new TelevisionRatingsV4 table keyed on a synthetic `id` (String, HASH).
 * This script ONLY reads from the source table and writes into the new v4 table —
 * TelevisionRatingsV3 is never modified or deleted.
 *
 * Run with: node src/scripts/migrateTvToIdTable.js
 * Verify only: node src/scripts/migrateTvToIdTable.js --verify
 * Force re-migration over a non-empty target table: node src/scripts/migrateTvToIdTable.js --force
 */
...
const TABLES = {
  SOURCE: 'TelevisionRatingsV3',
  TARGET: 'TelevisionRatingsV4',
};
...
function findMatchingNewItem(oldItem, newItems) {
  return newItems.find((newItem) => newItem.title === oldItem.title && newItem.date === oldItem.date);
}
...
async function runVerify() {
  ...
  for (const oldItem of oldItems) {
    ...
    if (mismatches.length > 0) {
      mismatchCount += 1;
      console.error(`❌ Mismatch for "${oldItem.title}":`);
...
async function runMigration({ force }) {
  console.log('\n🚀 Starting TV-to-id-table migration...');
  ...
  console.log(`🔄 Transformed ${newItems.length} TV records`);

  if (newItems.length > 0) {
    await batchWriteItems(TABLES.TARGET, newItems);
  }

  console.log('✅ TV migration completed');
...
async function main() {
  ...
  console.log('📺 Starting TV Reviews Migration to id-keyed Table');
  console.log('⚠️  This script only READS from TelevisionRatingsV3 and writes to TelevisionRatingsV4');
  console.log('⚠️  TelevisionRatingsV3 will not be modified or deleted\n');
