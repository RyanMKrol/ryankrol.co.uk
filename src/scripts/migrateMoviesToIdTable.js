/**
 * Migration script to copy movie review data from MovieRatingsV3 (keyed on title)
 * into a new MovieRatingsV4 table keyed on a synthetic `id` (String, HASH).
 * This script ONLY reads from the source table and writes into the new v4 table —
 * MovieRatingsV3 is never modified or deleted.
 *
 * Run with: node src/scripts/migrateMoviesToIdTable.js
 * Verify only: node src/scripts/migrateMoviesToIdTable.js --verify
 * Force re-migration over a non-empty target table: node src/scripts/migrateMoviesToIdTable.js --force
 */
...
const TABLES = {
  SOURCE: 'MovieRatingsV3',
  TARGET: 'MovieRatingsV4',
};
...
async function runMigration({ force }) {
  console.log('\n🚀 Starting Movies-to-id-table migration...');
  ...
  console.log(`🔄 Transformed ${newItems.length} movie records`);
...
async function main() {
  ...
  console.log('🎬 Starting Movie Reviews Migration to id-keyed Table');
  console.log('⚠️  This script only READS from MovieRatingsV3 and writes to MovieRatingsV4');
  console.log('⚠️  MovieRatingsV3 will not be modified or deleted\n');
