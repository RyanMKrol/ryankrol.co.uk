/**
 * Migration script to copy album review data from AlbumRatingsV2 (keyed on title+artist)
 * into a new AlbumRatingsV3 table keyed on a synthetic `id` (String, HASH).
 * This script ONLY reads from the source table and writes into the new v3 table —
 * AlbumRatingsV2 is never modified or deleted.
 *
 * Run with: node src/scripts/migrateAlbumsToIdTable.js
 * Verify only: node src/scripts/migrateAlbumsToIdTable.js --verify
 * Force re-migration over a non-empty target table: node src/scripts/migrateAlbumsToIdTable.js --force
 */
...
const TABLES = {
  SOURCE: 'AlbumRatingsV2',
  TARGET: 'AlbumRatingsV3',
};
...
async function runMigration({ force }) {
  console.log('\n🚀 Starting Albums-to-id-table migration...');
  ...
  console.log(`🔄 Transformed ${newItems.length} album records`);
...
async function main() {
  ...
  console.log('💿 Starting Album Reviews Migration to id-keyed Table');
  console.log('⚠️  This script only READS from AlbumRatingsV2 and writes to AlbumRatingsV3');
  console.log('⚠️  AlbumRatingsV2 will not be modified or deleted\n');
