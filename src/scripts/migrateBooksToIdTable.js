/**
 * Migration script to copy book review data from BookRatingsV3 (keyed on title+author)
 * into a new BookRatingsV4 table keyed on a synthetic `id` (String, HASH).
 * This script ONLY reads from the source table and writes into the new v4 table —
 * BookRatingsV3 is never modified or deleted.
 *
 * Run with: node src/scripts/migrateBooksToIdTable.js
 * Verify only: node src/scripts/migrateBooksToIdTable.js --verify
 * Force re-migration over a non-empty target table: node src/scripts/migrateBooksToIdTable.js --force
 */
...
const TABLES = {
  SOURCE: 'BookRatingsV3',
  TARGET: 'BookRatingsV4',
};
...
/**
 * Pure transform: copy every field from the old item unchanged and append a
 * freshly generated `id`. No renaming/dropping/transforming of existing fields —
 * title and author become ordinary non-key attributes on the new table.
 */
...
function findMatchingNewItem(oldItem, newItems) {
  return newItems.find((newItem) => newItem.title === oldItem.title && newItem.author === oldItem.author);
}
...
async function runVerify() {
  ...
  for (const oldItem of oldItems) {
    ...
    if (mismatches.length > 0) {
      mismatchCount += 1;
      console.error(`❌ Mismatch for "${oldItem.title}" by "${oldItem.author}":`);
...
async function runMigration({ force }) {
  console.log('\n🚀 Starting Books-to-id-table migration...');
  ...
  console.log(`🔄 Transformed ${newItems.length} book records`);
...
async function main() {
  ...
  console.log('📚 Starting Book Reviews Migration to id-keyed Table');
  console.log('⚠️  This script only READS from BookRatingsV3 and writes to BookRatingsV4');
  console.log('⚠️  BookRatingsV3 will not be modified or deleted\n');
