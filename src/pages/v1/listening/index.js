...
export default function V1ListeningPage() {
  ...
  function toggleSort(key) {
    if (sortKey === key) {
      ...
      setSortKey(key);
      setSortDir(key === 'name' || key === 'artist' ? 'asc' : 'desc');
    }
  }
    ...
    <V1Layout breadcrumb="~ / listening">
      <div className="v1-review-shell">
        <div className={`v1-table-pane${selected ? ' v1-table-pane-narrow' : ''}`}>
          ...
          {loading && <p className="v1-status">Loading listening data…</p>}
          ...
          {!loading && !error && (
            <>
              <table className="v1-table">
                <thead>
                  <tr>
                    <th onClick={() => toggleSort('rank')} className="v1-numeric">
                      #{sortArrow('rank')}
                    </th>
                    <th onClick={() => toggleSort('name')}>Album{sortArrow('name')}</th>
                    {!selected && <th onClick={() => toggleSort('artist')}>Artist{sortArrow('artist')}</th>}
                    <th onClick={() => toggleSort('playcount')} className="v1-numeric">
                      Plays{sortArrow('playcount')}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {pageRows.map((album, i) => (
                    <tr
                      key={`${album.artist}-${album.name}`}
                      className={i % 2 === 0 ? 'v1-row-even' : 'v1-row-odd'}
                      onClick={() => setSelected(album)}
                    >
                      <td className="v1-numeric">{album.rank || i + 1}</td>
                      <td>{album.name}</td>
                      {!selected && <td>{album.artist}</td>}
                      <td className="v1-numeric">{album.playcount}</td>
                    </tr>
                  ))}
                  {pageRows.length === 0 && (
                    <tr>
                      <td colSpan={selected ? 3 : 4} className="v1-status">
                        No albums found.
                      </td>
                    </tr>
                  )}
          ...
          <div className="v1-detail-panel">
            ...
            {selected.image && (
              <div className="v1-detail-row v1-detail-row-block">
                ...
                <img src={selected.image} alt={selected.name} width={160} height={160} />
              </div>
            )}
            ...
            <div className="v1-detail-row">
              <span className="v1-detail-label">Rank</span>
              <span>{selected.rank || '—'}</span>
            </div>
            <div className="v1-detail-row">
              <span className="v1-detail-label">Plays</span>
              <span>{selected.playcount}</span>
            </div>
            {selected.url && (
              <div className="v1-detail-row">
